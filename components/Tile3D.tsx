...
const Tile3D: React.FC<Tile3DProps> = ({ tile }) => {
  ...
  // Configuration de ressort plus "snappy" pour la réactivité
  const springConfig = {
    tension: 280,
    friction: 30,
    precision: 0.001
  ...
  const { pos, scale, color, emissiveIntensity } = useSpring({
    pos: position,
    // Effet de pop : si c'est une fusion, on agrandit légèrement
    scale: tile.isMerged ? [1.15, 1.15, 1.15] : [1, 1, 1],
    color: TILE_COLORS[tile.value] || '#3c3a32',
    emissiveIntensity: tile.value >= 128 ? 1.8 : 0.6,
    from: {
  ...
  return (
    <animated.group position={pos} scale={scale}>
      ...
      <Text
        ...
        fontSize={tile.value >= 1024 ? 0.32 : tile.value >= 100 ? 0.38 : 0.5}
